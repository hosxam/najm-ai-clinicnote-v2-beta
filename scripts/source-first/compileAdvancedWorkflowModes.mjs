...
const workflowEntries = activeCatalog.map((summary) => {
  ...
  return {
    ...
    schema: {
      ...
      advanced: { fields: advancedFields, chip_groups: chipGroups, history_placeholders: history.editable_placeholders ?? [], exam_groups: exam.exam_groups.length, exam_prompts: examPromptCount, investigation_groups: investigations.investigation_groups?.length ?? 0, investigation_options: investigationOptionCount, plan_groups: plan.plan_option_groups?.length ?? 0, plan_options: planOptionCount, medication_groups: medicationGroupCount, medication_options: medicationOptionCount, confirmation_required: true, preselected_facts: false },
    },
...
const output = {
  ...
  contract: { quick_engine: 'src/pages/QuickNotePage.tsx', advanced_engine: 'src/pages/DetailedEncounterPage.tsx', chip_engine: 'src/components/ChipSelector.tsx', output_engine: 'src/lib/outputBuilders.ts', active_only: true, inactive_route_policy: 'fail_closed' },
