...
async (page) => {
  ...
  const expectedBuild = '__EXPECTED_BUILD__'
  const errors = []
  const failed = []
  const requests = []
  ...
  const obsolete = await page.evaluate(async (url) => (await fetch(url)).status, `${base}data-beta/curated-workflows/catalog.json?deep-pilot=${Date.now()}`)
  const routeResults = []
  for (const id of activeIds) {
    await page.goto(`${base}#/beta/workflows/${id}`, { waitUntil: 'networkidle' })
    const text = await page.locator('body').innerText()
    const quick = await page.getByRole('button', { name: 'Quick' }).count()
    const advanced = await page.getByRole('button', { name: 'Advanced' }).count()
  ...
  for (const id of inactiveIds) {
    await page.goto(`${base}#/beta/workflows/${id}`, { waitUntil: 'networkidle' })
    const text = await page.locator('body').innerText()
    inactiveResults.push({ workflow_id: id, failed_closed: /inactive and is not available as usable clinical content/i.test(text), usable_marker: /\bUsable\b/i.test(text) })
  }
  const responsive = []
  ...
  const result = {
    ...
    local_filesystem_paths_exposed: /[A-Z]:\\|C:\\\\Users|file:\/\//i.test(home) === false
  }
