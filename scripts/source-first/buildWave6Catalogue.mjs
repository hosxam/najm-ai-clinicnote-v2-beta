...
for (const target of targets) {
  ...
  const field = { workflow_id: target.workflow_id, field_id: fieldId, archetype: target.archetype, section, label, helper_text: 'Enter only clinician-assessed information for this workflow; leave blank when not assessed.', field_type: type, options: type === 'investigation_result' || type === 'plan_entry' ? [] : [`Clinician-entered ${specificToken} context`, 'Not assessed'], free_text_allowed: true, required: false, display_order: workflow.fields.length + 1, visibility: { type: 'always' }, contradictory_option_rules: type === 'investigation_result' || type === 'plan_entry' ? [] : [{ group_id: `${target.workflow_id}__state`, mutually_exclusive_values: [`Clinician-entered ${specificToken} context`, 'Not assessed'] }], population_restrictions: [], setting_restrictions: [], soap_destination: soap, note_template: `${label}: {{value}}`, value_formatter: 'trimmed_text', provenance: { evidence_pack_ids: workflow.evidence_pack_ids, evidence_statement_ids: [evidenceId], source_ids: source?.source_id ? [source.source_id] : [], population: target.population, setting: target.setting, restrictions: [], support_level: 'workflow_specific_documentation_support', exact_section_id: exactSection.section_id, transformation_explanation: 'Clinician-entered value only; no diagnosis, prescription, interpretation or referral is inferred.' } }
