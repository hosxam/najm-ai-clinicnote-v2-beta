...
const terminalFailClosedStatuses = new Set(['no_authoritative_basis_after_full_search', 'blocked_source_access', 'superseded_source'])
const terminalPacks = packs.filter((pack) => pack.pack_status === 'completed' || terminalFailClosedStatuses.has(pack.pack_status))
const result = { status: errors.length ? 'FAIL' : 'PASS', families: manifest.guideline_family_count, packs: packs.length, completed: packs.filter((pack) => pack.pack_status === 'completed').length, terminal_fail_closed: packs.filter((pack) => terminalFailClosedStatuses.has(pack.pack_status)).length, pending: packs.length - terminalPacks.length, evidence_statements: packs.reduce((total, pack) => total + pack.evidence_statements.length, 0), corpus_fingerprint: manifest.corpus_fingerprint, aggregate_fingerprint: manifest.aggregate_fingerprint, errors }
