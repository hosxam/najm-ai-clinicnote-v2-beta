...
const buildFields = (row) => {
  ...
  return specs.map(([fieldId, label, fieldType, section, required, options]) => {
    ...
    return {
      ...
      field_id: field,
      label,
      field_type: fieldType === 'select' ? 'single_select' : fieldType,
      ...
      provenance: {
        evidence_pack_ids: [`wave11-pack-${id}`],
        evidence_statement_ids: [`wave11-${id}-statement-${String(specs.indexOf(specs.find((spec) => spec[0] === fieldId)) + 1).padStart(3, '0')}`],
        workflow_id: id,
        field_id: field,
