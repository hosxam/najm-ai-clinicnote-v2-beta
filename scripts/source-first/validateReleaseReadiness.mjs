...
if (coverage.active_workflow_count !== 877) errors.push('active workflow count is not 877')
if (coverage.records.some((row) => row.status !== 'exact_source_complete')) errors.push('active coverage contains a non-terminal status')
...
if (matrix.active_workflow_count !== 877 || matrix.unknown_or_pending_count !== 0) errors.push('release matrix contains unknown or pending workflows')
if (Object.values(matrix.final_status_counts).reduce((sum, value) => sum + value, 0) !== 877) errors.push('release matrix status totals do not reconcile')
...
if (errors.length) {
  ...
  console.log(JSON.stringify({ status: 'PASS', denominator: 1500, active: 877, fields: provenance.field_count, uaeFindings: 1426, legacyRuntimeIncluded: 0, releaseStatusCounts: matrix.final_status_counts }, null, 2))
}
