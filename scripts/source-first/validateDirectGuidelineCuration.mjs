...
const errors = []
if (workflows.length !== 1500 || metadata.workflow_count !== 1500 || catalog.length !== metadata.usable_workflow_count) errors.push('workflow accounting or usable beta count mismatch')
const ids = new Set()
...
if (catalog.length && (removed !== metadata.counts.removed || added !== metadata.counts.added || retained !== metadata.counts.retained || rewritten !== metadata.counts.rewritten)) errors.push('metadata counts do not match detail records')
...
if (metadata.usable_workflow_count !== metadata.fully_reconstructed_workflows + metadata.workflows_with_documented_limitations) errors.push('unavailable workflows included in usable beta')
if (metadata.unavailable_workflow_count !== metadata.source_gap_workflows + metadata.blocked_source_workflows + metadata.incomplete_workflows) errors.push('unavailable workflow accounting mismatch')
