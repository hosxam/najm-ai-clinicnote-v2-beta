...
write('FINAL_CATALOGUE_DENOMINATOR_MODEL.json', { schema_version: '1.0.0', authoritative: true, source_catalogue_counts: finalManifest.counts, denominator_counts: denominatorCounts, totals: { original_records: denominator.length, true_distinct_clinical_workflows: denominator.filter((row) => ['active_distinct_clinical_workflow', 'inactive_distinct_missing_authoritative_evidence', 'blocked_by_source_access'].includes(row.final_denominator_classification)).length, active_distinct_workflows: denominator.filter((row) => row.final_denominator_classification === 'active_distinct_clinical_workflow').length, remaining_targetable_distinct_workflows: denominator.filter((row) => row.final_denominator_classification === 'inactive_distinct_missing_authoritative_evidence').length, evidence_blocked_distinct_workflows: denominator.filter((row) => row.final_denominator_classification === 'blocked_by_source_access').length }, records: denominator, fingerprint: hash(denominator) })
