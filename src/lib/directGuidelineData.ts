...
export type CuratedDetail = { workflow_number: number; workflow_id: string; title: string; specialty: string; diagnosis: string; source_status: string; source_grounded: boolean; content: Record<string, CuratedItem[]>; sources_used: CuratedSource[]; additions: CuratedItem[]; rewrites: CuratedItem[]; removals: RemovedItem[]; source_limitations: string[]; curation_policy: string }
export type CuratedCatalogEntry = Pick<CuratedDetail, 'workflow_number' | 'workflow_id' | 'title' | 'specialty' | 'diagnosis' | 'source_status' | 'source_grounded'> & { sources_used: number; retained_count: number; rewritten_count: number; removed_count: number; added_count: number; unresolved_source_gap_count: number }
export type CuratedMetadata = { schema_version: string; dataset: string; generated_from: string; workflow_count: number; item_count: number; workflows_with_guideline_evidence: number; workflows_without_guideline_evidence: number; counts: { retained: number; rewritten: number; removed: number; added: number }; source_count: number; production_data_path: string; clinician_review_queue: false }
