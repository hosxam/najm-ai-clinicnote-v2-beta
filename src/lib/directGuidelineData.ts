...
export type CuratedItem = { item_id: string; section_id: string; text: string; action: 'add' | 'retain' | 'rewrite'; rationale: string; evidence_extract: string | null; source: CuratedSource & { exact_section: { section_id: string; heading: string; locator: string } | null } }
...
export type CuratedSectionCoverage = { applicable: boolean; status: 'intentionally_inapplicable' | 'covered_by_committed_evidence' | 'missing_full_source_review'; evidence_item_count: number }
export type CuratedDetail = { workflow_number: number; workflow_id: string; title: string; specialty: string; diagnosis: string; source_status: string; source_grounded: boolean; source_review_basis: string; full_guideline_documents_inspected: false; fully_reconstructed: false; completeness_status: string; applicable_sections: string[]; section_coverage: Record<string, CuratedSectionCoverage>; content: Record<string, CuratedItem[]>; sources_used: CuratedSource[]; additions: CuratedItem[]; rewrites: CuratedItem[]; removals: RemovedItem[]; source_limitations: string[]; curation_policy: string }
...
export type CuratedMetadata = { schema_version: string; dataset: string; generated_from: string; source_review_basis: string; full_guideline_documents_inspected: false; workflow_count: number; item_count: number; workflows_with_guideline_evidence: number; workflows_without_guideline_evidence: number; fully_reconstructed_workflows: number; incomplete_workflows: number; counts: { retained: number; rewritten: number; removed: number; added: number }; source_count: number; newly_registered_sources: number; production_data_path: string; clinician_review_queue: false }
