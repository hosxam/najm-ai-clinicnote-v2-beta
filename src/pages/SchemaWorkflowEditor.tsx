...
export function SchemaWorkflowEditor() {
  ...
  useEffect(() => {
    ...
    setWorkflow(null); setValues({}); setSavedDraft(null); setDraftChoice(null); setOutput(''); setProcedureOutput(''); setError(null)
  ...
  const reset = () => { setValues({}); setOutput(''); setProcedureOutput(''); localStorage.removeItem(draftKey(workflowId, 'quick')); localStorage.removeItem(draftKey(workflowId, 'advanced')) }
  const generate = () => { if (!workflow) return; setOutput(buildInteractiveSoapNote(workflow, values)); setProcedureOutput(buildInteractiveProcedureNote(workflow, values)) }
