...
function StructuredFieldControl({ field, value, onChange }: { field: InteractiveField; value: string; onChange: (value: string) => void }) {
  ...
  return <div className="grid gap-2 sm:grid-cols-2" id={field.field_id} role="group" aria-label={field.label}>
    {schema.map(({ key, label }) => <label key={key} className="grid gap-1 text-xs text-slate-600"><span>{label}</span>{key === 'status' || key === 'certainty' || key === 'comparison' || (key === 'unit' && field.field_type !== 'investigation_result') ? <select className="field-input" value={current[key] ?? ''} onChange={(event) => update(key, event.target.value)} aria-label={`${field.label} ${label}`}><option value="">Not assessed</option>{(key === 'status' ? ['Normal', 'Abnormal', 'Present', 'Absent', 'Not assessed', 'Unknown'] : key === 'certainty' ? ['Verified', 'Unverified', 'Unknown'] : key === 'comparison' ? ['No prior result', 'Stable', 'Improved', 'Worsened', 'New', 'Unknown'] : key === 'unit' && field.field_type === 'duration' ? ['hours', 'days', 'weeks', 'months', 'years'] : key === 'unit' && field.field_type === 'vital_sign' ? ['°C', 'bpm', '/min', '%', 'mmHg', 'kg', 'cm', 'mmol/L', 's', 'score', 'L/min', 'room air', 'Unknown'] : []).map((option) => <option key={option} value={option}>{option}</option>)}</select> : <Input value={current[key] ?? ''} placeholder={label} aria-label={`${field.label} ${label}`} onChange={(event) => update(key, event.target.value)} />}</label>)}
  </div>
}
