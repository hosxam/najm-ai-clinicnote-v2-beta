import { ArrowLeft, BrainCircuit, CheckCircle2, ExternalLink, Layers3 } from 'lucide-react'
...
export function BetaWorkflowModePage() {
  ...
  return <div className="min-w-0 space-y-5">
    <header className="rounded-3xl border border-cyan-200 bg-slate-950 bg-cover bg-center p-5 text-white shadow-xl sm:p-6" style={{ backgroundImage: `linear-gradient(90deg, rgba(2, 6, 23, 0.97) 0%, rgba(2, 6, 23, 0.86) 48%, rgba(8, 47, 73, 0.62) 100%), url(${publicPath('assets/najm-constellation-hero.png')})` }}>
      <div className="flex flex-wrap items-center justify-between gap-3"><Link to="/beta" className="inline-flex items-center gap-2 text-sm font-semibold text-cyan-100 hover:text-white"><ArrowLeft size={16} /> Catalogue</Link><span className="font-mono text-xs text-cyan-200">Build {buildMarker}</span></div>
