  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <span className={cn('flex h-11 w-11 items-center justify-center rounded-xl', primary ? 'bg-white/12' : 'bg-slate-100 text-cyan-800')}>
          <Icon className="h-5 w-5" />
        </span>
        {label ? (
          <span className={cn('rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em]', primary ? 'bg-white/12 text-cyan-50' : 'bg-slate-100 text-slate-600')}>
            {label}
          </span>
        ) : null}
      </div>
      <div className="mt-8">
        ...
        <p className={cn('mt-2 max-w-md text-sm leading-6', primary ? 'text-cyan-50/75' : 'text-slate-600')}>{description}</p>
        <span className={cn('mt-5 inline-flex items-center gap-2 text-sm font-semibold', primary ? 'text-white' : 'text-cyan-800')}>
